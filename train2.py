...
def pixel_accuracy(labels, predicted):
    num_correct_pixels = np.sum(labels == predicted)
    tot_pixels = labels.size
    return num_correct_pixels / tot_pixels

def mean_IOU(labels, predicted):
    iou_sum = 0.0
    for i in range(num_classes):
        intersection = np.logical_and(labels == i, predicted == i).sum()
        union = np.logical_or(labels == i, predicted == i).sum()
        if union == 0:
            iou_sum += 1.0
        else:
...
def frequency_IOU(labels, predicted):
    class_counts = np.zeros(num_classes)
    for i in range(num_classes):
        class_counts[i] = np.sum(labels == i)
    
    freq_weights = class_counts / np.sum(class_counts)
    ...
    for i in range(num_classes):
        ...
        union = np.logical_or(labels == i, predicted == i).sum()
        if union == 0:
            weighted_iou += freq_weights[i] * 1.0
        else:
            weighted_iou += freq_weights[i] * (intersection / union)
    return weighted_iou
